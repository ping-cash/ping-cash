...
export async function complianceRoutes(fastify: FastifyInstance) {
  fastify.post('/sanctions/screen/wallet', async (request: FastifyRequest, reply: FastifyReply) => {
    const body = ScreenWalletBody.parse(request.body);
    const result = await screenWallet(body);
    return reply.status(200).send(result);
  });

  fastify.post('/sanctions/screen/name', async (request: FastifyRequest, reply: FastifyReply) => {
    const body = ScreenNameBody.parse(request.body);
    const result = await screenName(body);
    return reply.status(200).send(result);
  });

  fastify.post('/transfers/check-allowance', async (request: FastifyRequest, reply: FastifyReply) => {
