...
| #22                     | EPIC: Phase 2 — $PING TGE + Foundation + audit + Raydium | IN PROGRESS | claude | All 4 scaffolds reviewed + ADR 0021 catalog + 45 scaffold fixes / 77 findings + verifier ZERO DRIFT × 2. **#62 rebuild started: internal-swap H-04 Pyth integration helpers shipped (a92e86f assert_amm_within_oracle_band + cf1f6be read_pyth_ping_per_usdc_x64)** — pyth-sdk-solana=0.10.5 dep pinned; band-check + staleness/confidence guards; helpers stand alone until swap instructions wire them in (next ABI-additive step). #61 (earn-vault) + #62 (internal-swap+pomm) cover Anchor architectural remainder. Cayman + OtterSec founder gates. |
