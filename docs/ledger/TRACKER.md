...
**Last refreshed:** 2026-05-27 (post backlog-sweep session — #58 Wise adapter + #16 earn-vault-svc closed; #61 batch-1 earn-vault Anchor source-side fixes shipped). **Commits this session:** a162dac (Wise real 4-call flow + 15 vitest), 0d1ee1a (sub-agent reviewer 8 wire-format fixes), 096046e (earn-vault-svc Fastify scaffold + GET /users/:userId/vault + stub-aware reader), 834ced8 (earn-vault #61 batch-1 — squads multisig scaffold + solvency invariant + emergency_withdraw closing H-02 + H-04 + H-05). **Issue flips:** #58 closed (PARTIAL → PASS after fixes); #16 closed (scaffold complete; activation gated on #15 program); #61 in-progress (batch-1 of N — adapter CPIs + UserShare PDA remain toolchain-bound per runbook 584c169). **Workspace health:** 19/19 pnpm packages typecheck clean; 37+ vitest passing in offramp + 3 passing in earn-vault. **CI status:** Build & Deploy / Anchor / Corridor-smoke all still red on #54 GHA billing — Mech-4 podman+ghcr remains the working image-build path (validated previously, this session's TS-only changes need Mech-4 rebuild → see follow-up).
...
| ⏸️ `status/parked`      | 6     | #54 GHA billing (mech-4 mitigated), #50 vendor keys (founder-paste), #15/#23/#24 Phase-2 mainnet (audit + cayman), #62 internal-swap+pomm rebuild (toolchain-bound) |
