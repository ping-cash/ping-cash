...
export type CashOutMethod =
  | 'gcash' | 'maya' | 'bdo-bank' | 'bpi-bank' | 'unionbank' | 'metrobank' | 'cebuana-cash-pickup'
  | 'upi' | 'neft-bank' | 'paytm' | 'phonepe' | 'gpay-india'
  | 'jazzcash' | 'easypaisa' | 'hbl-bank' | 'mcb-bank' | 'ubl-bank'
  | 'bkash' | 'nagad' | 'rocket' | 'bd-bank'
  | 'm-pesa' | 'airtel-money' | 'kcb-bank' | 'equity-bank'
  | 'turkish-bank'
  | 'sepa' | 'sepa-instant' | 'uk-faster-payments' | 'us-ach' | 'us-wire'
  | 'bank-transfer';

export type ProviderName = 'transfi' | 'wise' | 'flutterwave' | 'yellow-card' | 'thunes' | 'cebuana';
