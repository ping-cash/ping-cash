...
async function requireAuth(
  ...
  const payload = (request.server as {
...
export async function walletRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/balance', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const walletAddress = (request.query as { address?: string }).address ?? auth.wallet;
    if (!walletAddress) {
      return reply.status(400).send({
        error: { code: 'VALIDATION_ERROR', message: 'No wallet address available' },
      });
    }
  ...
  fastify.get('/address', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!auth.wallet) {
      return reply.status(404).send({
        error: { code: 'WALLET_NOT_FOUND', message: 'No wallet bound to this account' },
      });
    }
  ...
  fastify.get('/vault', async (request: FastifyRequest, reply: FastifyReply) => {
    const auth = await requireAuth(request, reply);
    if (!auth.wallet) {
      return reply.status(404).send({
  ...
  fastify.get('/validate', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!address) {
      return reply.status(400).send({
        error: { code: 'VALIDATION_ERROR', message: 'address query param required' },
      });
    }
    return reply.status(200).send({ valid: isValidSolanaAddress(address), address });
  });
}
