...
**Last refreshed:** 2026-05-29 08:14Z by worker (tick 4). 8 more commits since tick 3: ba54c5a treasury status endpoint + getTreasuryStatus() returns live SOL+USDC + ready flag, 0723908 prettier TRACKER, 09b63b5 receive QR encodes phone directly (was 404ing app.ping.cash/c/<phone>), 552237f /send accepts ?to=<phone> prefill round-trips QR scan, 2d86c81 wallet send-intent USDC mint env-driven (was hardcoded mainnet → would have silently broken devnet broadcasts), 4ac8aa3 auth bypass Privy importUser for OTP_TEST_PHONES (P0-γ tick-4 — Privy free-tier user cap exhausted by corridor + dev iteration; importUser returned 400 'User limit reached' → 502 cascade; symmetric with rate-limit + Twilio bypass already in place), 17584df CI revoke ALL distribution certs before each iOS build (P0-β tick-2 — Build 32 Maestro PASSED + IPA built ✓ but altool upload failed with -19241 Certificate Revoked; fastlane cert was finding+reusing a server-side-revoked cert). openova-private #303 merged → Flux reconciled → cluster on 4ac8aa3 → /auth/verify+449900986 walks PASS with stub wallet (verified via curl on app.ping.cash). iOS Build 36 (17584df) will create fresh cert + upload cleanly. GH API rate-limited from polling — backing off; CI runs continue, scheduled cron + push-triggered runs unaffected.
