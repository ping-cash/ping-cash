...
describe('fee.service.computeFee — mobile-wallet on $400', () => {
  ...
  it('bronze + USDC: $2.00 ($1.20 provider + $0.80 markup)', () => {
    ...
    expect(parseFloat(b.providerCostUsd)).toBeCloseTo(1.2, 4);
    expect(parseFloat(b.pingMarkupAfterPayInPingUsd)).toBeCloseTo(0.8, 4);
    expect(parseFloat(b.totalFeeUsd)).toBeCloseTo(2.0, 4);
  ...
  it('silver + USDC: $1.60 ($1.20 + $0.40)', () => {
    ...
    expect(parseFloat(b.totalFeeUsd)).toBeCloseTo(1.6, 4);
    expect(parseFloat(b.savingsVsBronzeUsd)).toBeCloseTo(0.4, 4);
  });
  ...
  it('silver + pay-in-PING: $1.30 ($1.20 + $0.10)', () => {
    ...
    expect(parseFloat(b.totalFeeUsd)).toBeCloseTo(1.3, 4);
    expect(parseFloat(b.savingsVsBronzeUsd)).toBeCloseTo(0.7, 4);
  });
...
describe('fee.service.computeFee — bank-transfer on $400', () => {
  ...
  it('bronze: $3.00 ($1.60 + $1.40)', () => {
    ...
    expect(parseFloat(b.totalFeeUsd)).toBeCloseTo(3.0, 4);
    expect(parseFloat(b.providerCostUsd)).toBeCloseTo(1.6, 4);
  });
...
describe('fee.service.computeFee — cash-pickup on $400', () => {
  ...
  it('bronze: $4.00 ($2.80 + $1.20)', () => {
    ...
    expect(parseFloat(b.totalFeeUsd)).toBeCloseTo(4.0, 4);
  });
...
describe('fee.service.computeFee — phpc-direct on $400', () => {
  ...
  it('bronze: $0.40 ($0.20 + $0.20)', () => {
    ...
    expect(parseFloat(b.totalFeeUsd)).toBeCloseTo(0.4, 4);
  });
...
describe('fee.service.applyHardFloor', () => {
  it('clamps total fee at provider cost (defense in depth)', () => {
    ...
    expect(parseFloat(floored.totalFeeUsd)).toBeGreaterThanOrEqual(
      parseFloat(floored.providerCostUsd)
    );
  });
});
...
describe('fee.service — Maria simulation (per BUSINESS-STRATEGY.md)', () => {
  ...
  it('12-month cumulative fee at Silver+pay-in-PING = $15.60', () => {
    ...
    expect(total).toBeCloseTo(15.6, 1);
  });
  ...
  it('vs Bronze full fee over 12 months = $24.00, saves $8.40', () => {
    ...
    expect(total).toBeCloseTo(24.0, 1);
  });
});
