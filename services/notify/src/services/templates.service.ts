...
> = {
  TRANSFER_RECEIVED: {
    whatsapp: p =>
      ...
      `Expires in 7 days.`,
    sms: p =>
      `Ping: You received ${p.amount} from ${p.senderName}. Claim at ${p.claimUrl}`,
    push: p => ({
  ...
  TRANSFER_SENT: {
    whatsapp: p =>
      ...
      `Status: ${p.statusUrl}`,
    sms: p =>
      `Ping: ${p.amount} sent to ${p.recipientName}. Ref ${p.reference}`,
    push: p => ({
      title: `${p.amount} sent`,
  ...
  CLAIM_REMINDER: {
    ...
    sms: p =>
      `Ping reminder: claim ${p.amount} at ${p.claimUrl} — expires ${p.hoursRemaining}h`,
    push: p => ({
  ...
  CASHOUT_COMPLETE: {
    ...
    sms: p =>
      `Ping: ${p.localAmount} ${p.localCurrency} delivered to your ${p.method}. Ref ${p.reference}`,
    push: p => ({
  ...
  WELCOME_STAKE_GRANTED: {
    ...
    sms: _p =>
      `Ping: 1,200 $PING earned! You're now Silver tier — 50% off fees.`,
    push: _p => ({
  ...
  MILESTONE_UNLOCKED: {
    ...
    sms: p =>
      `Ping: 200 $PING unlocked (${p.milestone})! New balance: ${p.newBalance}`,
    push: p => ({
